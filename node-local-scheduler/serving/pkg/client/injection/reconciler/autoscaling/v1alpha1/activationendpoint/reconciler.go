...
// controller reconciling v1alpha1.ActivationEndpoint.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1alpha1.ActivationEndpoint. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1alpha1.ActivationEndpoint) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.ActivationEndpoint.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1alpha1.ActivationEndpoint. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1alpha1.ActivationEndpoint) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1alpha1.ActivationEndpoint if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1alpha1.ActivationEndpoint.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1alpha1.ActivationEndpoint) reconciler.Event
}

// ReadOnlyFinalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.ActivationEndpoint if they want to process tombstoned resources
...
type ReadOnlyFinalizer interface {
	// ObserveFinalizeKind implements custom logic to observe the final state of v1alpha1.ActivationEndpoint.
	// This method should not write to the API.
	ObserveFinalizeKind(ctx context.Context, o *v1alpha1.ActivationEndpoint) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1alpha1.ActivationEndpoint) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1alpha1.ActivationEndpoint resources.
type reconcilerImpl struct {
	...
	Lister autoscalingv1alpha1.ActivationEndpointLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister autoscalingv1alpha1.ActivationEndpointLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.ActivationEndpoints(s.namespace)
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, existing *v1alpha1.ActivationEndpoint, desired *v1alpha1.ActivationEndpoint) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.AutoscalingV1alpha1().ActivationEndpoints(desired.Namespace)
		...
		updater := r.Client.AutoscalingV1alpha1().ActivationEndpoints(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.ActivationEndpoint) (*v1alpha1.ActivationEndpoint, error) {

	getter := r.Lister.ActivationEndpoints(resource.Namespace)
	...
	patcher := r.Client.AutoscalingV1alpha1().ActivationEndpoints(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.ActivationEndpoint) (*v1alpha1.ActivationEndpoint, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.ActivationEndpoint, reconcileEvent reconciler.Event) (*v1alpha1.ActivationEndpoint, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
